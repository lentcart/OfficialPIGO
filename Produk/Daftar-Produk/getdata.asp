...
<%
    pdNama = request.queryString("cari")
    ' pdSku = request.queryString("cari")


    set produk_cmd = server.createObject("ADODB.COMMAND")
	produk_cmd.activeConnection = MM_PIGO_String
			
	produk_cmd.commandText = "SELECT * FROM [PIGO].[dbo].[MKT_M_Produk] where  pdNama like '%"& pdNama &"%' and pd_custID like '%"& request.Cookies("custID") &"%'  " 
                ...
                <input readonly class="tx-card" type="text" name="pdNama" id="pdNama" value="<%=produk("pdNama")%>"><br>
                    ...
                    <div class="row mt-2 " style="color:black; font-weight:bold; font-size:9px">
                        ...
                        <div class="col-3">
                            ...
                            <%end if%>
                            <%
                                StokAkhir_cmd.commandText = "SELECT MKT_M_Produk.pdStok-COUNT(MKT_T_Transaksi_D1A.tr_pdQty) AS total FROM MKT_M_Produk RIGHT OUTER JOIN  MKT_T_Transaksi_D1A ON MKT_M_Produk.pdID = MKT_T_Transaksi_D1A.tr_pdID RIGHT OUTER JOIN  MKT_T_Transaksi_D1 ON LEFT(MKT_T_Transaksi_D1A.trD1A, 12) = LEFT(MKT_T_Transaksi_D1.trD1, 12) RIGHT OUTER JOIN MKT_T_Transaksi_H ON LEFT(MKT_T_Transaksi_D1.trD1, 12) = MKT_T_Transaksi_H.trID where (MKT_T_Transaksi_D1A.tr_pdID = '"&  produk("pdID") &"') group by  MKT_M_Produk.pdStok,MKT_T_Transaksi_D1A.tr_pdQty"
