...
<%
	...
	report_cmd.commandText = "SELECT dbo.MKT_T_Transaksi_H.tr_custID, dbo.MKT_M_Customer.custNama, dbo.MKT_M_Customer.custEmail, dbo.MKT_M_Customer.custPhone1, dbo.MKT_M_Alamat.almLengkap FROM dbo.MKT_M_Customer INNER JOIN dbo.MKT_T_Transaksi_H ON dbo.MKT_M_Customer.custID = dbo.MKT_T_Transaksi_H.tr_custID INNER JOIN dbo.MKT_M_Alamat ON dbo.MKT_T_Transaksi_H.tr_almID = dbo.MKT_M_Alamat.almID LEFT OUTER JOIN dbo.MKT_T_Transaksi_D1 ON dbo.MKT_T_Transaksi_H.trID = LEFT(dbo.MKT_T_Transaksi_D1.trD1, 12) where MKT_T_Transaksi_D1.tr_slID = '"& request.cookies("custID") &"' AND MKT_T_Transaksi_D1.tr_strID = '03' and year(trTglTransaksi)='"& tahun &"' "& FilterFix & filterTanggal & "GROUP BY dbo.MKT_T_Transaksi_H.tr_custID, dbo.MKT_M_Customer.custNama, dbo.MKT_M_Customer.custEmail, dbo.MKT_M_Customer.custPhone1, dbo.MKT_M_Alamat.almLengkap"
