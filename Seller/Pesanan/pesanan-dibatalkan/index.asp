...
<span class="txt-Judul" >Pesanan  Dibatalkan </span>
    <table class="table table-bordered table-condensed mt-2" style="font-size:11px">
        ...
        <tr>
            <th colspan="12"> Customer : <%=Transaksi("custNama")%> - [ <%=Transaksi("custEmail")%> ]</th>
        </tr>
        <%
        ...
        <tr>
            ...
            <td class=" text-center" ><input class=" text-center" type="text" name="" id="" value="<%=Replace(FormatCurrency(total),"$"," Rp. ")%>"></td>
            <% if pdtr("strID") = "00" then%>
            <td class=" text-center" ><span class="stpesanan00"><%=pdtr("strName")%></span></td>
            <%else%>
            <td class=" text-center" ><span class="stpesanan01"><%=pdtr("strName")%></span></td>
            <%end if%>
            <td class=" text-center" ><%=pdtr("trPengiriman")%></td>
            <td class=" text-center" ><a href="pesanan-baru/detail.asp?trID=<%=pdtr("trID")%>" style="font-size:11px"> Periksa Rincian Pesanan </a></td>
        </tr>
        <%pdtr.movenext
                    loop%>
        <%Transaksi.movenext
