...
<% 
    ...
    psTglBayar = request.form("TglBayar")

    set order_CMD = server.CreateObject("ADODB.command")
    order_CMD.activeConnection = MM_pigo_STRING

    order_CMD.commandText = "exec sp_add_MKT_T_Pesanan '"& ps_trID &"','"& ps_tglTransaksi &"',"& ps_trQty &",'"& ps_pdCustID &"','"& ps_pdID &"',"& ps_trOngkir &",'"& ps_trJenisPengiriman &"','"& ps_custID &"',"& ps_trSubtotal &",'"& ps_trJenisPembayaran &"','1','0','0','0','0'"
    ...
    ' delete_CMD.commandText = "DELETE FROM [dbo].[MKT_T_Transaksi_H] WHERE cart_custID = '"& ps_custID  &"' and cart_pdID = '"& ps_pdID &"'"
