...
<%
    ...
    if Periode = "1" then 
        ...
        set KartuStok_CMD = server.createObject("ADODB.COMMAND")
        KartuStok_CMD.activeConnection = MM_PIGO_String
        KartuStok_CMD.commandText = "SELECT MKT_T_SAPDB.SAPDB_pdID, "& SAPDB_Pembelian &" AS Pembelian, "& SAPDB_HargaPembelian &" AS HargaPembelian, "& SAPDB_Penjualan &" AS Penjualan, "& SAPDB_HargaPenjualan &" AS HargaPenjualan, MKT_T_SAPDB.SAPDB_SaldoAwal , SAPDB_HargaSaldoAwal , MKT_M_PIGO_Produk.pdNama, SAPDB_Tanggal AS Tanggal ,MKT_M_PIGO_Produk.pdPartNumber FROM MKT_T_SAPDB LEFT OUTER JOIN MKT_M_PIGO_Produk ON MKT_T_SAPDB.SAPDB_pdID = MKT_M_PIGO_Produk.pdID WHERE SAPDB_pdID = '"& pdID &"' AND SAPDB_Tahun = '"& YEAR(Tgla) &"' AND SAPDB_Tanggal BETWEEN '"& tgla &"' AND '"& tgle &"' ORDER BY SAPDB_Tanggal ASC"
        'Response.Write KartuStok_CMD.commandText
        set KartuStok = KartuStok_CMD.execute
    end if 
                    ...
                    <div class="col-12">
                        <table class="table vertical-align-center cont-tb table-bordered table-condensed mt-1" style="font-size:11px;">
                            ...
                            <tbody>
                                ...
                                %>
                                <tr>
                                    <td class="text-center"> <%=KartuStok("Tanggal")%> </td>
                                    <!--SaldoAwal-->
                                        <td class="text-center"> <%=KartuStok("SAPDB_SaldoAwal")%> </td>
                                        <td class="text-end"> <%=Replace(replace(FormatCurrency(KartuStok("SAPDB_HargaSaldoAwal")),"$","Rp. "),".00","")%> </td>
                                        <td class="text-end"> 
                                            <% JumlahSaldoAwal = KartuStok("SAPDB_SaldoAwal")*KartuStok("SAPDB_HargaSaldoAwal") %>
                                    ...
                                    <!--Pembelian-->
                                        <% 
                                            Sisa            = KartuStok("SAPDB_SaldoAwal")+KartuStok("Pembelian")
