...
<%
	...
	GL_M_ChartAccount_cmd.activeConnection = MM_PIGO_String
        GL_M_ChartAccount_cmd.commandText = "SELECT CA_ID, CA_Name FROM GL_M_ChartAccount WHERE CA_Name Like '%"& CA_Name &"%' AND NOT  CA_Name LIKE  '%n/a%' AND NOT CA_Type = 'H'  "
        ...
        <% do while not AccountKas.eof %>
            ...
            <script>
                function getAccountKas<%=AccountKas("CA_ID")%>(){
                    $.ajax({
                        type: "get",
                        url: "get-Account2.asp?CA_ID="+document.getElementById("CA_ID<%=AccountKas("CA_ID")%>").value,
