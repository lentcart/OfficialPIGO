...
<%
    ...
    GL_M_ChartAccount_cmd.commandText = "SELECT CA_ID, CA_Name FROM GL_M_ChartAccount  WHERE CA_Name Like '%"& CA_Name &"%' AND CA_AktifYN = 'Y' AND NOT  CA_Name LIKE  '%n/a%' AND NOT CA_Type = 'H' AND CA_ItemTipe <> 'C' AND CA_ID <> 'A100.02.00' AND CA_UpID <> 'A100.02.00'"
    set AccountKas = GL_M_ChartAccount_cmd.execute 
    else 
    set GL_M_ChartAccount_cmd = server.createObject("ADODB.COMMAND")
	GL_M_ChartAccount_cmd.activeConnection = MM_PIGO_String
    GL_M_ChartAccount_cmd.commandText = "SELECT CA_ID, CA_Name FROM GL_M_ChartAccount   WHERE CA_Name Like '%"& CA_Name &"%' AND CA_AktifYN = 'Y' AND NOT  CA_Name LIKE  '%n/a%' AND NOT CA_Type = 'H' "
    set AccountKas = GL_M_ChartAccount_cmd.execute 
    end if
