...
<% 
    ...
    Transaksi_H_CMD.commandText = "Select custNama From MKT_M_Customer Where custID = '"& request.cookies("custID") &"' "
    'response.write Transaksi_H_CMD.commandText &"<br><br>"
    set customername = Transaksi_H_CMD.execute 
    ...
    for i = 0 to Ubound(sellerid)
        ...
        NotifID     = "02"
        NotifType   = "Pesanan Baru"
        ExpDate     = left(Pay_Expired,10)
        Tanggal     = CDate(left(ExpDate,10))
        Tgl         = Day(Cdate(Tanggal))&"-"&Month(Cdate(Tanggal))&"-"&year(Cdate(Tanggal))
        NotifDesc   = "Pesanan "& IDTransaksi &" Atas Nama "& customername("custNama") &"perlu membayar sebelum "& Tgl &" Tunggu konfirmasi pembayaran & pastikan stok produk masih tersedia. Jika tidak, segera chat Pembeli"

        set Notifikasi_CMD = server.CreateObject("ADODB.command")
        Notifikasi_CMD.activeConnection = MM_pigo_STRING
        Notifikasi_CMD.commandText = "exec sp_add_MKT_M_Notifikasi_D  '"& NotifID &"','"& NotifType &"', '"& NotifDesc &"', 'N', '"& sellerid(i) &"' "
